...
# Auto-delivery on payment; caption is short & professional; NO buttons
...
def build_upi_uri(amount: float, note: str):
    # encode + no decimals unless needed (for QR payload only; not shown)
...
def start_purchase(ctx: CallbackContext, chat_id: int, uid: int, item_id: str):
    ...
    note = f"order_uid_{uid}"
    uri = build_upi_uri(amt, note)   # used to build the QR only
    img = qr_url(uri)
